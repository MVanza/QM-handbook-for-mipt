...
\hspace{1em} На прошлом семинаре мы обсуждали возможность измерения наблюдаемых, используя собственные значения и собственные функции. Это связано с тем, что наблюдаемое имеет нулевую неопределенность, если состояние является для него собственным: 
\[
...
\begin{enumerate}
    ...
    \item Если же они коммутируют, то можно показать, что действие одного из операторов на собственный вектор второго само по себе является собственным состоянием: $\hat{A} \ket{a_i} = a_i \ket{a_i} \implies \hat{B} \hat{A} \ket{a_i} = a_i \hat{B} \ket{a_i} \implies \hat{A} ( \hat{B} \ket{a_i} ) = a_i ( \hat{B} \ket{a_i} )$. Далее, если собственное значение не вырождено (для вырожденного случая докажите самостоятельно), то $\hat{B} \ket{a_i}$ и $\ket{a_i}$ пропорциональный друг другу. А это значит, что вектор $\ket{a_i}$ собственный и для $\hat{B}$.
\end{enumerate}
...
Давайте посмотрим на квадрат волновой функции де Бройля: $|\braket{x}{p}|^2 = \frac{1}{2 \pi \hbar}$. Если мы возьмём интеграл от этой константы по всему пространству, то он будет бесконечен. Дело в том, что, как уже говорилось ранее, собственные состояния оператора координаты и импульса нефизичны, так как мы предполагаем, что знаем импульс точно. Реальные же состояния нормированные, и модуль квадрата будет соответствовать вероятности. Давайте разберёмся, откуда у нас появляется этот нормировочный множитель, вычислив $\braket{p}{p'}$ через $\braket{x}{x'}$ (далее я буду опускать пределы интеграла, кроме случаев, где они неочевидны):
\begin{align*}
...
\end{gather*}
Давайте подробно разберём вторую строчку. Первый член получим, взяв производную от скалярной функции. Получаем её вторую производную и нормированый вектор. Второй член -- применяем оператор импульса (то есть градиент) к вектору. Третий член полуается из следующих соображений (напомню, что $r = \sqrt{x^2 + y^2 + z^2}$ и $\frac{\partial f(r)}{\partial x} = \frac{\partial f(r)}{\partial r} \frac{\partial r}{\partial x}$):
\[
...
\]
Подставляем $f(r) = 1/r$ и получаем третий член. Последнего слагаемое получилось путём сложения левого и правого слагаемого в первой строчке перед последним равно.
\csquare{darklavender}
