...
Давайте применим полученные нами знания на практике. Попробуем решить радиальное уравнение для атома водорода. В атоме водорода электрон движется в электростатическом потенциале, создаваемом тяжёлым ядром. Потенциал имеет следующий вид:
\[
...
\[
\left[ -\frac{\hbar^2}{2Mr^2}\pd{}{r}(r^2 \pd{}{r}) + \frac{\hbar^2 l(l+1)}{2Mr^2} - \frac{e^2}{r}\right]R_{nl}(r) = ER_{nl}(r).
\]
Сделаем замену переменной $R_{nl}=U_{nl}(r)/r$ и перепишем уравнение через новую переменную:
\begin{align*}
\frac{1}{r^2}\pd{}{r}\left[ r^2 \pd{}{r} R_{nl} \right] &= \frac{1}{r^2}\pd{}{r}\left[ r^2 \pd{}{r} \frac{U_{nl}}{r} \right] = \frac{1}{r^2}\pd{}{r}\left[ r^2 \left(\frac{U'_{nl}}{r} - \frac{U_{nl}}{r^2}\right) \right] = \frac{U''_{nl}}{r} \implies \\
...
\]
Это уравнение Коши -- Эйлера. Решим его с заменой $U_{nl} = r^\alpha$):
\[
...
\]
где $n$ --- некоторое натуральное число, а $A_{j+1} \neq 0$ --- действительные коэффициенты. 

Найдём коэффициент $\kappa$, рассмотрев этот ряд и радиальное уравнение при $r \rightarrow +\infty$. В этом случае доминировать будут члены с максимальной степенью, то есть правая часть уравнения. Найдём производную ряда:
\[
...
С коэффициентами $A$ разобрались, осталось найти условие на $n$. Может появиться вопрос: почему мы вообще решили, что сумма должна быть конечна? Оказывается, что получившийся ряд расходится, если $n=\infty$. Действительно, при больших $j$ 
\[
A_j \sim \frac{(2\kappa)^j}{j!},
\] 
и тогда функция имеет вид
\[
...
2\kappa n = \frac{2}{a},
\]
где все $A_j$ при $j > n$ обнуляются.

Теперь мы знаем, как найти радиальные волновые функции для конкретных значений $n$ и $l$. Попробуем это сделать в виде упражнения.
\newpage
\begin{exercise}
...
R_{10}(r) = A_1e^{-r/a}.
\]
Выведем значение коэффициента $A_1$ из нормировки волновой функции:
\begin{align*}
...
\]
c) Для последнего случая $n=2,\, l=1$ остаётся только коэффициент $A_2$. Тогда радиальная волновая функция выглядит так:
\[
R_{21}(r) = A_2re^{-r/2a}.
\]
Нормируя, получим
...
\]
Посмотрев на получившиеся радиальные волновые функции, мы понимаем, в чём физический смысл боровского радиуса: он определяет характерный размер волновых функций, а также примерный радиус орбитали основного состояния.
\qedsq
...
\end{itemize}
Получается, что при заданном главном квантовом числе $n$, вырождение определяется как $2n^2$. Действительно, количество значений $l$ равно $n$, степень вырождения $m$ в свою очередь $2l+1$. Тогда, посчитав сумму первых $n$ членов арифметической прогрессии, получим 
\[
\sum\limits_{l=0}^{n-1}(2l +1 ) = n^2. 
\]
Двойка появится, если мы учтём вырождение по спину: он может принимать два значения --- $\pm 1/2$.
...
Дипольный момент характеризует переход атома из одного состояния в другое после излучения фотона. Значит, остаться на одном и том же уровне электрон никак не может, --- если фотон излучился, то энергия должна уменьшится. Давайте рассмотрим это на одной из проекций. Напомню, что $r_j = \begin{pmatrix}
...
\end{gather*}
То же самое будет и с остальными проекциями состояния на самих себя. При проекции на состоянии с разными $l$ интеграл будет не нулевой. Ниже я рассматриваю только переход от $n=2$ к $n=1$, так как дипольный момент характеризуется поглощением или испусканием фотона, поэтому энергия системы обязана измениться.

Посмотрим на переходы между энергетическими уровнями. Давайте распишем это в общем виде:
\begin{align*}
...
\end{align*}
где $I_r(n,\,l,\,n',\,l')$ и $I_{\theta\phi}(l,\,m,\,l',\,m')$ --- радиальный и угловой интеграл.
\bigskip
\newlineСоответственно, получается следующее:
\begin{gather*}
...
\]
Теперь посчитаем угловые интегралы. Убедимся, что переход между уровнем $\bra{1,\,0,\,0}\mathbf{r}\ket{2,\,0,\,0}$ невозможен.
\begin{align*}
...
Теперь найдём возможные переходы:
\begin{align*}
    ...
    & = \frac{\sqrt{3}}{4\pi}\int\limits_{0}^{\pi} \begin{pmatrix} \sin^2\theta \cos\theta\\ \sin^2\theta \cos\theta \\ \sin\theta\cos^2\theta \end{pmatrix} d\theta \int\limits_{0}^{2\pi} \begin{pmatrix} \cos\phi \\ \sin\phi \\ 1 \end{pmatrix} d\phi = \\
    ...
    & = -\frac{\sqrt{3}}{4\pi\sqrt{2}}\int\limits_{0}^{\pi} \begin{pmatrix} \sin^3 \theta \\ \sin^3 \theta \\ \sin^2\theta\cos\theta \end{pmatrix} \, d\theta \int\limits_{0}^{2\pi} \begin{pmatrix} e^{i\phi}\cos\phi \\ e^{i\phi}\sin\phi \\ e^{i\phi} \end{pmatrix} \, d\phi = \\
...
\begin{align*}
 ...
 \bra{1,\, 0,\, 0}&y\ket{2,\, 1,\, \pm 1} = - i\frac{2^7}{3^5}a;\\
 \bra{1,\, 0,\, 0}&z\ket{2,\, 1,\,  0} = \frac{2^7 \sqrt{2}}{3^5}a.
\end{align*}
\qedsq