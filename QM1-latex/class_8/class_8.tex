\section{Семинар \thesection}\label{sem:angular-momentum}
\subsection{Собственные значения момента импульса}
Предыдущий семинар мы закончили на радиальном уравнении:
\[
\left[-\frac{\hbar^2}{2Mr^2}\pd{}{r}\left( r^2 \pd{}{r}\right)  + \frac{\lambda}{2Mr^2} + V(r)\right]R(r) = ER(r).
\]
...
\begin{exercise}\label{ex:angular-momentum-matrices}
...
\begin{exercise}
    ...
    \]
    \[
    (\hat{\Vec{\sigma}}\Vec{a})(\hat{\Vec{\sigma}}\Vec{b}) = (\Vec{a}\Vec{b}) + i(\Vec{\sigma}[\Vec{a} \times \Vec{b}]).
    \]
\end{exercise}
...
\begin{align*}
(\hat{\Vec{\sigma}}\Vec{a})(\hat{\Vec{\sigma}}\Vec{b}) &= \hat{\sigma}_ka_k\hat{\sigma}_lb_l = \hat{\sigma}_k\hat{\sigma}_la_kb_l= \\
&=(i\varepsilon_{klm}\hat{\sigma}_m + \delta_{kl})a_kb_l =\\
&= i\varepsilon_{klm}\hat{\sigma}_m a_k b_l + a_k b_k =\\
&= i(\hat{\sigma}[\Vec{a} \times \Vec{b}]) + (\Vec{a}\Vec{b}).
\end{align*}
\qedsq
