...
\]
Напомню, что такое линеаризованный потенциал $V_{lin}(x)$:
\[
...
\]
Пользуясь полученным выше соотношением, выведем следующий результат:
\begin{gather*}
...
\end{gather*}
Получается, что при значениях $n\geq 125$ линейная аппроксимация и асимптотика функции Эйри не будут превышать ошибку в $1\%$. В квазиклассических системах значение $n$ сильно больше.
...
\end{cases}
\]
Запишем линеаризованный потенциал (обратим внимание на то, что в этот раз первая производная будет отрицательной, так как потенциал убывает) как $V(x) \approx E + V'(0)x$. Тогда уравнение Шрёдингера будет иметь следующий вид:
\[
    \frac{d^2\psi_T}{dx^2} = -\alpha^3 x\psi_p, \quad \alpha = \left( \frac{2m|V'(0)|}{\hbar^2} \right)^{1/3}.
\]
Решением этого дифференциального уравнения будут функции Эйри:
\[
...
\]
Ещё раз сравнив две волновые функции, получим:
\[
...
\]
Воспользуемся этими значениями в первоначальной функции для $x>0$:
\[
...
\end{cases}
\]
Внутри барьера запрещённая область, поэтому импульс стоит под модулем. Слева приходит обычная волна и улетает отражённая, поэтому экспоненты две. Справа же только волна, которая прошла сквозь барьер, поэтому слагаемое строго одно. 
...
\end{cases}
\]
Подставив их в формулу для $A$, получим искомую связь между $A$ и $F$:
\[
A = \left( \frac{C}{2} - iD \right)e^{i\frac{\pi}{4}} = i\left( \frac{e^{-\gamma}}{4} - e^{\gamma} \right)F.
\]
Коэффициент прохождения --- отношение коэффициента прошедшей к коэффициенту падающей волны $T = \frac{|F|^2}{|A|^2}$. Найдём его в нашей задаче:
\[
