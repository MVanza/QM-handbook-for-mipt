...
\]
Правая часть ямы очень похожа на яму из упражнения \textnumero\,12. Рассмотрим, что происходит у левой стенки. Мы считаем, что частица не может пройти через такой бесконечный барьер, то есть $\psi(x) = 0$, $x\leq 0$. Тогда из условия непрерывности получим условия «сшивки» (напоминаю, решения в области II имеет вид $\psi(x) = \widetilde{A}\cos kx + A\sin kx$):
\[
...
\end{cases}
\]
Теперь мы можем переписать это через нечётные волновые функции из упражнения \textnumero\,12. Тогда получим
\[
\psi(x) =
\begin{cases}
    0, & x\leq 0,\\
...
Давайте для начала рассмотрим области вне дельта-ям. Ясно, что решением здесь будут так же чётные и нечётные функции. Для проверки этого факта достаточно проделать те же самые выкладки, что мы делали в упражнении \textnumero\,12. Выпишем эти функции:
\[
\psi_+ =
\begin{cases}
    A e^{\kappa x}, & x < -a,\\
...
\end{enumerate}
Найдём коэффициенты для собственных функций (так же, как в упражнении \textnumero\,12).
\[
