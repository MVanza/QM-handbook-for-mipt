...
Всё хорошее когда-нибудь заканчивается и настаёт пора суровой реальной жизни. Так и для нас наступило время переходить к трёхмерному пространству. Давайте попробуем разобраться, как изменится подход к описанию систем такого типа.
...
\[
J = \left|\pd{(x, y, z)}{(r, \theta, \phi)}\right| = r^2\sin\theta.
\]
Тогда скалярное произведение запишется в виде
\[
\braket{\psi}{\varphi} = \int\limits_{-\infty}^{+\infty}\int\limits_{-\infty}^{+\infty}\int\limits_{-\infty}^{+\infty}\psi^*(r)\varphi(r)dx\,dy\,dz= \int\limits_0^{2\pi}\int\limits_0^{\pi}\int\limits_0^{\infty}\psi^*(r)\varphi(r)r^2\sin\theta dr \,d\theta\,d\phi.
\]
...
Для удобства записи вспомним, что такое \textit{символ Леви-Чивиты}:
\[
\varepsilon_{ijk} = 
\begin{cases}
...
\]
Далее свернём $\varepsilon_{jmn}\delta_{nk} = \varepsilon_{jmk}$ и, так как символ Леви-Чивиты антисимметричный, заменим $\varepsilon_{jmk} = -\varepsilon_{jkm}$. В итоге получим
\[
...
Сейчас придётся помучиться с индексами, но нам не впервой, так что посчитаем третий коммутатор:
\begin{align*}
...
\end{align*}
Теперь давайте попробуем пойти обратным путём, то есть выразить полученную формулу через произведение символов Леви-Чивиты. Ведь мы хотим подобрать индексы так, чтобы в итоге можно было представить выражение как символ Леви-Чивиты, умноженный на оператор момента импульса.
\[
...
\]
Индексы k и l --- немые, поэтому мы можем менять их, как хотим. Давайте поменяем их местами. С одной стороны, это нечётная перестановка, значит, $\varepsilon_{jkl} = -\varepsilon_{jlk}$. С другой стороны, операторы координаты коммутируют, следовательно, $2i\hbar\varepsilon_{jkl}\hat{r}_k\hat{r}_l = 2i\hbar\varepsilon_{jlk}\hat{r}_k\hat{r}_l$. Получается, что единственный вариант исполнения этих условий — выражение равно нулю.
\[
...
\]
Последнее выражение играет важную роль, потому что в системе могут быть одновременно определены и компонента момента импульса, и его квадрат.
\qedsq
...
\]
Вывести это уравнение можно, если расписать более подробно $\hat{L}^2$ как $\hat{L}_j\hat{L}_k = (\varepsilon_{jlm}\hat{r}_l\hat{p}_m)(\varepsilon_{knq}\hat{r}_n\hat{p}_q)$. Умножив стационарное уравнение Шрёдингера на $\hat{r}^2$ и заменив $\hat{r}^2\hat{p}^2$ на $\hat{L}^2 + (\hat{\Vec{r}}\hat{\Vec{p}})^2 - i\hbar\hat{\Vec{r}}\hat{\Vec{p}}$ , получим:
\[
...
\]
Можно заметить, что у нас разделились переменные: первое слагаемое влияет только на радиальную часть, второе --- на вращательную. Третье тоже на радиальную, если потенциал вращательно инвариантен, то есть $V(\Vec{r}) = V(r)$. Но это пока не показано строго, так как мы работаем в сферических координатах. Теперь мы перейдём к ним.
\begin{exercise}
...
\[
\hat{L}_j = -i\hbar\varepsilon_{jkl}r_k\pd{}{r_l}.
\]
...
\[
\pd{}{z} = \pd{}{r}\pd{r}{z} + \pd{}{\theta}\pd{\theta}{z} + \pd{}{\phi}\pd{\phi}{z}.
\]
...
\begin{equation*}
    ...
    \end{cases}
\end{equation*}
Тогда дифференцируя и выражая результат в сферических координатах, найдём:
\begin{gather*}
\pd{r}{z} = \frac{z}{\sqrt{x^2 + y^2 + z^2}} = \cos\theta;\\
\pd{\theta}{z} = \frac{-(x^2+y^2)}{\sqrt{\frac{x^2 + y^2}{x^2+y^2+z^2}}(x^2+y^2+z^2)^{3/2}}= -\frac{r^2\sin^2\theta}{\sqrt{\frac{r^2\sin^2\theta}{r^2}}r^3} = -\frac{\sin\theta}{r}; \\
\pd{\phi}{z} = 0.
\end{gather*}
Выпишу значение производных по оставшимся компонентам, чтобы читатели могли проверить себя:
\begin{gather*}
\pd{r}{x} = \sin\theta\cos\phi; \; \pd{\theta}{x} = \frac{1}{r}\cos\theta\cos\phi; \; \pd{\phi}{x} = -\frac{1}{r}\frac{\sin\phi}{\sin\theta}.\\
\pd{r}{y} = \sin\theta\sin\phi; \; \pd{\theta}{y} = \frac{1}{r}\cos\theta\sin\phi; \; \pd{\phi}{y} = \frac{1}{r}\frac{\cos\phi}{\sin\theta}.
\end{gather*}

Теперь, зная все необходимые частные производные, подставим их в изначальное уравнение и получим
\begin{gather*}
\pd{}{x} = \sin\theta\cos\phi\pd{}{r} + \frac{1}{r}\cos\theta\cos\phi\pd{}{\theta} -\frac{1}{r}\frac{\sin\phi}{\sin\theta}\pd{}{\phi};\\
\pd{}{y} = \sin\theta\sin\phi\pd{}{r} + \frac{1}{r}\cos\theta\sin\phi\pd{}{\theta} +\frac{1}{r}\frac{\cos\phi}{\sin\theta}\pd{}{\phi};\\
\pd{}{z} = \cos\theta\pd{}{r} -\frac{\sin\theta}{r}\pd{}{\theta}.
\end{gather*}
Подставим эти значения в компоненты момента импульса:
\begin{align*}
\hat{L}_z &= -i\hbar(x\pd{}{y} - y\pd{}{x}) =\\
&= -i\hbar\left(r\sin\theta\cos\phi\left(\sin\theta\sin\phi\pd{}{r} + \frac{1}{r}\cos\theta\sin\phi\pd{}{\theta} +\frac{1}{r}\frac{\cos\phi}{\sin\theta}\pd{}{\phi}\right) - \dots \right).
\end{align*}
Можно заметить, что $r\sin^2\theta \cos\phi \sin\phi \pd{}{r} - r\sin^2\theta\cos\phi\sin\phi\pd{}{r} = 0$. Второй член аналогично сократится. Тогда можно записать, что
\begin{align*}
    \hat{L}_z &= -i\hbar\left(r\sin\theta\cos\phi\frac{1}{r}\frac{\cos\phi}{\sin\theta}\pd{}{\phi} + r\sin\theta\sin\phi\frac{1}{r}\frac{\sin\phi}{\sin\theta}\pd{}{\phi} \right) =\\ 
    &= -i\hbar \left(\cos^2\phi+\sin^2\phi\right)\pd{}{\phi} = -i\hbar\pd{}{\phi}.
\end{align*}
Аналогично для других компонент:
\begin{gather*}
\hat{L}_x = i\hbar\left(\sin\phi\pd{}{\theta} + \ctg\,\theta\cos\phi \pd{}{\phi}\right);\\
\hat{L}_y = i\hbar\left(-\cos\phi\pd{}{\theta} + \ctg\,\theta\sin\phi \pd{}{\phi}\right).
\end{gather*}
...
\begin{gather*}
\hat{\Vec{r}}\hat{\Vec{p}} = -i\hbar r\pd{}{r};\\
(\hat{\Vec{r}}\hat{\Vec{p}})^2 = -\hbar^2\left( r^2\frac{\partial^2}{\partial r^2} + r\pd{}{r}\right).
\end{gather*}
\qedsq
...
Получается, из всего многообразия таких состояний нам необходимо отобрать те, которые являются собственными для гамильтониана. Для этого подставим найденные в сферических координатах операторы в уравнение Шрёдингера, запишем волновую функцию как $\psi(r, \theta, \phi) = R(r)Y_{\lambda}(\theta, \phi)$ и заменим оператор $\hat{L}^2$ на его собственное значение: 
\[
\left[-\frac{\hbar^2}{2M}\left( r^2\frac{\partial^2}{\partial r^2} + 2r\pd{}{r} \right)  + \frac{\lambda}{2M} + r^2V(r)\right]R(r)Y_{\lambda}(\theta, \phi) = r^2ER(r)Y_{\lambda}(\theta, \phi).
\]
Тогда, воспользовавшись тем, что 
\[
r^2\frac{\partial^2}{\partial r^2} + 2r\pd{}{r} = \pd{}{r}\left( r^2 \pd{}{r}\right),
\]
...
Какой полезный вывод можно сделать? Оказывается, если радиальная часть волновой функции $\psi(r, \theta, \phi) = R(r)Y_{\lambda}(\theta, \phi)$ удовлетворяет радиальному уравнению, то эта волновая функция является собственной для гамильтониана с собственным значением E. Таким образом, мы разделили задачу на две: нахождение собственных значений $\hat{L}^2$ (приведение к диагональному виду) и решение диффура. Первую часть задачи достаточно решить один раз. Этим мы и займёмся в следующем семинаре.