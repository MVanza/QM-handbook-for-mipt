...
\[
i\hbar\pd{}{t}\psi(\Vec{r}_1, \Vec{r}_2, t) = \hat{H}\psi(\Vec{r}_1, \Vec{r}_2, t),
\]
...
\begin{gather*}
\pd{}{r_1} = \pd{}{R}\pd{R}{r_1} + \pd{}{r}\frac{\partial r}{r_1} = \pd{}{R}\frac{m_1}{m_1 + m_2} + \pd{}{r};\\
\pd{}{r_2} = \pd{}{R}\frac{m_2}{m_1 + m_2} - \pd{}{r}.
\end{gather*}
Теперь, когда у нас есть эти выражения, можем найти связь между вторыми производными. Для этого воспользуемся следующим фактом:
\[
\frac{\partial^2}{\partial r_1^2} = \pd{}{r_1}(\pd{}{R}\frac{m_1}{m_1 + m_2} + \pd{}{r}) = (\pd{}{R}\frac{m_1}{m_1 + m_2} + \pd{}{r})(\pd{}{R}\frac{m_1}{m_1 + m_2} + \pd{}{r}).
\]
Запишем вторую производную в явном виде:
\begin{align*}
\frac{\partial^2}{\partial r_1^2} &= \frac{\partial^2}{\partial R^2} \frac{\partial^2 R}{\partial r_1^2} + 2\frac{\partial^2}{\partial R\,\partial r}\pd{R\,\partial r}{r_1^2} + \frac{\partial^2}{\partial r^2} \frac{\partial^2 r}{\partial r_1^2} = \\
...
\end{align*}
\begin{align*}
    \frac{\partial^2}{\partial r_2^2} &= \frac{\partial^2}{\partial R^2} \frac{\partial^2 R}{\partial r_2^2} + 2\frac{\partial^2}{\partial R\,\partial r}\pd{R\,\partial r}{r_2^2} + \frac{\partial^2}{\partial r^2} \frac{\partial^2 r}{\partial r_2^2} = \\
...
\[
\hat{H} = -\frac{\hbar^2}{2M}\Delta_{R} - \frac{\hbar^2}{2\mu}\Delta_{r} + U(\Vec{r}).
\]
...
\end{align*}
Из первого уравнения следует, что функция $\Phi(\Vec{R})$ описывает свободное движение центра масс: $\Phi(\Vec{R}) = Ce^{\frac{i}{\hbar}\Vec{P}\Vec{R}}$, характеризующееся полным импульсом $P$ и энергией $E_{\text{цм}} = \Vec{P}^2/2M$.
\qedsq

\begin{exercise}
    \textit{Частица массы $m$ движется в потенциале трёхмерного изотропного осциллятора:}
    \[
...
Запишем уравнение Шрёдингера для трёхмерного изотропного осциллятора:
\[
\left[-\frac{\hbar^2}{2m}\Delta + \frac{m\omega^2 r^2}{2}\right]\psi(\Vec{r}) = E\psi(\Vec{r}).
\]

В декартовой системе данная задача была решена в Семинаре \ref{sem:oscillator}, поэтому сейчас посмотрим, что происходит при использовании сферических координат. Сначала выпишем явный вид лапласиана:
\[
\Delta = \frac{1}{r^2}\pd{}{r}\left(r^2\pd{}{r}\right) + \frac{1}{r^2 \sin\theta}\pd{}{\theta}\left(\sin\theta\pd{}{\theta}\right) + \frac{1}{r^2\sin^2\theta}\frac{\partial^2}{\partial\phi^2}.
\]

В Семинаре \ref{sem:angular-momentum} в упражнении \textnumero\,\ref{ex:angular-momentum-matrices} было показано, что оператор квадрата момента импульса $\hat{L}^2$ выглядит так:
\[
\hat{L}^2 = -\hbar^2\left[\frac{1}{\sin\theta}\pd{}{\theta}\left( \sin\theta\pd{}{\theta} \right) + \frac{1}{\sin^2\theta}\frac{\partial^2}{\partial\phi^2}\right].
\]
Написанное в квадратных скобках похоже на два последних слагаемых лапласиана, поэтому можем его преобразовать:
\[
\Delta = \frac{1}{r^2}\pd{}{r}\left(r^2\pd{}{r}\right) - \frac{\hat{L}^2}{\hbar^2 r^2} = \frac{1}{r^2}\left(2r\pd{}{r} + r^2 \frac{\partial^2}{\partial r^2}\right) - \frac{\hat{L}^2}{\hbar^2 r^2} = \frac{\partial^2}{\partial r^2} + \frac{2}{r}\pd{}{r} - \frac{\hat{L}^2}{\hbar^2 r^2}.
\]

Поработаем теперь над уравнением Шрёдингера:
\begin{align*}
     -\frac{\hbar^2}{2m}\frac{\partial^2\psi}{\partial r^2} - \frac{\hbar^2}{mr}\pd{\psi}{r} + \frac{\hat{L}^2}{2mr^2}\psi + \frac{m\omega^2 r^2}{2}\psi - E\psi & = 0 \; \Bigg| \cdot r^2 \implies \\
    \implies -\frac{\hbar^2 r^2}{2m}\frac{\partial^2\psi}{\partial r^2} - \frac{\hbar^2 r}{m}\pd{\psi}{r} + \frac{\hat{L}^2}{2m}\psi + \frac{m\omega^2 r^4}{2}\psi - Er^2\psi & = 0.
\end{align*}
...
\]
Обезразмерим данное уравнение, перейдя от $r$ к $z = \frac{r}{\rho}$, где $\rho = \sqrt{\frac{\hbar}{m\omega}}$, и посмотрим на асимптотическое поведение искомой радиальной функции. Найдём связь между старыми и новыми производными:
\begin{gather*}
...
\end{gather*}
Полученные ряды можно подставить в радиальное уравнение:
\begin{gather*}
...
Заметим, что $z$ входит в сумму с двумя разными показателями степени. Анализировать выражение в таком виде неудобно, поэтому сведём данную сумму к привычному виду степенного ряда:
\begin{gather*}
...
\end{gather*}
Так как это уравнение должно быть верным для любого $z$, занулим все коэффициенты данного ряда. Теперь $a_1 = 0$. Для остальных членов запишем:
\begin{align*}
...
\end{align*}
В такой формулировке становится понятно, что все нечётные коэффициенты $a_k$ тождественно равны нулю. Заметим, что при больших $k$ коэффициенты $a_{k+2} \approx -\frac{a_k}{k}$, что делает наш ряд расходящимся. Получается, мы должны «обрубить» ряд, обнуляя все слагаемые, начиная с некоторого $k = n_k = 0, 2, 4 \dots$:
\begin{gather*}
...
\]
Выразив все коэффициенты через нулевой и подставив их в сумму, находим нулевой элемент с помощью нормировки. Для большего удобства переобозначим $k\rightarrow2k$, $n_k\rightarrow2n_k$ и выпишем решение радиального уравнения вместе с выражением для энергии:
\begin{align*}
...
Для того, чтобы посчитать кратность вырождения, нужно вспомнить, что в исходном уравнении Шрёдингера была ещё и угловая часть, для нахождения которой нужно выявить собственные функции оператора $\hat{L}^2$. Процесс поиска подробно описан в \nameref{appendix:A}. Здесь же выпишем только результат:
\[
Y^m_l(\theta, \phi) = \mathcal{N}_l\sqrt{\frac{(l+m)!}{(l-m)!}} \frac{1}{\sin^{m}\theta}\,\frac{d^{l-m}}{d(\cos\theta)^{l-m}}\sin^{2l}\theta e^{im\phi},
\]
где $\mathcal{N}_l = (-1)^l \sqrt{\frac{2l+1}{4\pi}}\frac{1}{2^l l!}$ --- нормировочный коэффициент, а $l$ и $m$ --- орбитальное и магнитное квантовые числа, соответственно.
\bigskip
\newlineВводя еще и главное квантовое число $n=2k+l=2n_k+l$, мы понимаем, что каждым фиксированным $n$ и $l$ соответствуют $2l+1$ разных состояний (так как $m$ --- целое число, удовлетворяющее системе неравенств $-l\leq m\leq l$). Заметим, что для чётных $n$ орбитальный момент $l = 0, 2, \dots, n-2, n$, а для нечётных $l = 1, 3, \dots, n-2, n$. Тогда кратность вырождения выглядит как
\[
...
\begin{exercise}
    \textit{Частица массы $m$ движется в трёхмерной потенциальной яме с «плоским» дном и бесконечно высокими стенками:}
    \[
    V(r)=
    \begin{cases}
        0,\; r<a;\\
...
Запишем уравнение Шрёдингера для данного потенциала. Оно, как мы знаем из предыдущих Семинаров, будет разделяться на угловую и радиальную часть. Таким образом, волновую функцию можно записать в виде $\Psi_{nlm}(r, \theta, \phi) = R_{nl}(r)Y^{m}_l(\theta, \phi) = \frac{1}{r}U_{nl}(r)Y^{m}_l(\theta, \phi)$, где $U_{nl}(r) = rR_{nl}(r)$. Подставляя её в радиальное уравнение, получим:
\begin{gather*}
...
1) Пусть $l = 0$. В таком случае уравнение Шрёдингера принимает вид
\begin{gather*}
...
\end{gather*}
Решением данного уравнения является линейная комбинация сферических функций Бесселя (обратите внимание, что мы перешли обратно к $U_{nl}(r)$, отсюда умножение на $\sqrt{r}$):
\[
...
Для начала разберём теорию. У частиц со спином есть так называемый собственный магнитный момент, который обозначается как $\Vec{\mu}$. Эта величина зависит от спинового момента $\Vec{S}$ следующим образом:
\[
\Vec{\mu} = \gamma \Vec{S}, \text{ где $\gamma$ --- это гиромагнитное отношение.}
\]
Когда частица со спином помещается в магнитное поле, она испытывает крутящий момент $\Vec{\mu} \times \Vec{B}$, который поворачивает её спин по полю. Энергию этого крутящего момента можно вычислить так:
\[
...
\]
Если забыли, откуда появилась такая матрица и что такое спин, --- вам в Семинар VIII. 
